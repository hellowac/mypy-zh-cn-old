...
[configuration file <config-file>] instead.
...
[attempt to locate <finding-imports>] that module
...
[PEP 561 compliant stub package <installed-packages>] (e.g. with a ``py.typed`` file) or have registered
...
2.  Searching to see if there is a [PEP 561 compliant stub package <installed-packages>]
...
3.  [Writing your own stub files <stub-files>] containing type hints for
    ...
    [stubgen <stubgen>], a program that comes bundled with mypy, to generate a first
...
2.  To suppress *all* missing import errors from a single library, add
    a per-module section to your [mypy config file <config-file>] setting
    ...
    [import discovery <config-file-import-discovery>] in config files.
...
see the [section below <missing-type-hints-for-third-party-library>].
...
- The directories containing the sources given on the command line
  (see [Mapping file paths to modules <mapping-paths-to-modules>]).
...
.. note
    ...
    installed (see [PEP 561 support <installed-packages>])
